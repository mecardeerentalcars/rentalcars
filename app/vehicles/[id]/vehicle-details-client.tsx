...
function EditVehicleForm({ data, onCancel, onSaved }: { data: VehicleProfilePayload; onCancel: () => void; onSaved: () => Promise<void> }) {
  ...
  return <div className={styles.editWrap}><div className={styles.editHead}><div><h2>Edit vehicle</h2><p>Update the same profile information used when adding a vehicle.</p></div></div>{error&&<div className={styles.error}>{error}</div>}
    <section className={styles.editSection}><h3>Vehicle details</h3><div className={styles.editGrid}>
      <label>Name<input value={base.name} onChange={e=>setBase({...base,name:e.target.value})}/></label><label>Make<input value={base.make} onChange={e=>setBase({...base,make:e.target.value})}/></label><label>Registration<input value={base.registrationNumber} onChange={e=>setBase({...base,registrationNumber:e.target.value.toUpperCase()})}/></label><label>Model year<input type="number" value={base.modelYear} onChange={e=>setBase({...base,modelYear:e.target.value})}/></label><label>Fuel<select value={base.fuelType} onChange={e=>setBase({...base,fuelType:e.target.value})}><option>Petrol</option><option>Diesel</option><option>Hybrid</option><option>Electric</option><option>CNG</option></select></label><label>Transmission<select value={base.transmission} onChange={e=>setBase({...base,transmission:e.target.value})}><option>Manual</option><option>Automatic</option></select></label><label>Daily rate<input type="number" value={base.dailyRate} onChange={e=>setBase({...base,dailyRate:e.target.value})}/></label><label>Odometer<input type="number" value={base.odometerKm} onChange={e=>setBase({...base,odometerKm:e.target.value})}/></label><label>Allowed KM/day<input type="number" value={base.allowedKmPerDay} onChange={e=>setBase({...base,allowedKmPerDay:e.target.value})}/></label><label>Extra KM rate<input type="number" value={base.extraKmRate} onChange={e=>setBase({...base,extraKmRate:e.target.value})}/></label><label>Mileage KM/L<input type="number" step="0.1" value={base.mileageKmPerLitre} onChange={e=>setBase({...base,mileageKmPerLitre:e.target.value})}/></label><label>Vehicle image<input type="file" accept="image/jpeg,image/png,image/webp" disabled={processing} onChange={e=>void chooseImage(e.target.files?.[0]??null)}/><small>{processing?"Compressing image…":"Automatically compressed before upload"}</small></label>{imagePreview&&<img className={styles.editPreview} src={imagePreview} alt="Vehicle preview"/>}
    </div></section>
    ...
    <section className={styles.editSection}><h3>Maintenance</h3><div className={styles.editGrid}><label>Title<input value={service.title} onChange={e=>setService({...service,title:e.target.value})}/></label><label>Due date<input type="date" value={service.dueDate} onChange={e=>setService({...service,dueDate:e.target.value})}/></label><label>Due KM<input type="number" value={service.dueOdometerKm} onChange={e=>setService({...service,dueOdometerKm:e.target.value})}/></label><label>Amount<input type="number" value={service.amount} onChange={e=>setService({...service,amount:e.target.value})}/></label><label className={styles.full}>Notes<input value={service.description} onChange={e=>setService({...service,description:e.target.value})}/></label></div></section>
    <section className={styles.editSection}><h3>Tyres</h3><div className={styles.editCards}>{tyres.map((tyre,i)=>{const label=TYRE_POSITIONS.find(([p])=>p===tyre.position)?.[1]??tyre.position;return <article key={tyre.position}><strong>{label}</strong><label>Brand<input value={tyre.brand} onChange={e=>setTyres(x=>x.map((t,n)=>n===i?{...t,brand:e.target.value}:t))}/></label><label>Model<input value={tyre.model} onChange={e=>setTyres(x=>x.map((t,n)=>n===i?{...t,model:e.target.value}:t))}/></label><label>Size<input value={tyre.size} onChange={e=>setTyres(x=>x.map((t,n)=>n===i?{...t,size:e.target.value}:t))}/></label><label>Replace by KM<input type="number" value={tyre.replacementDueOdometerKm} onChange={e=>setTyres(x=>x.map((t,n)=>n===i?{...t,replacementDueOdometerKm:e.target.value}:t))}/></label></article>})}</div></section>
