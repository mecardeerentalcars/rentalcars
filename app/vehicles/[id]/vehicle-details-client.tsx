...
function EditVehicleForm({ data, onCancel, onSaved }: { data: VehicleProfilePayload; onCancel: () => void; onSaved: () => Promise<void> }) {
  ...
  return <div className={styles.editWrap}><div className={styles.editHead}><div><h2>Edit vehicle</h2><p>Update the same profile information used when adding a vehicle.</p></div></div>{error&&<div className={styles.error}>{error}</div>}
    <section className={styles.editSection}><h3>Vehicle details</h3><div className={styles.operationalStatusEditor}><div><strong>Operational status</strong><small>{automaticStatusLocked ? "This status is controlled by the current rental." : "Set whether this vehicle is active, inactive, or in maintenance."}</small></div>{automaticStatusLocked ? <input value={v.status === "booked" ? "Booked (automatic)" : "On rent (automatic)"} disabled/> : <select value={operationalStatus} onChange={e=>setOperationalStatus(e.target.value)}><option value="available">Active</option><option value="inactive">Inactive</option>{!v.isGuest && <option value="maintenance">Maintenance</option>}</select>}</div><div className={styles.editGrid}>
