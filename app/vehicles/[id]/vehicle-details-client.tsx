...
type Vehicle = {
  ...
  extraKmRate: number; mileageKmPerLitre: number; status: string; isGuest: boolean; guestOwnerName: string | null; guestOwnerPlace: string | null; createdAt: string; updatedAt: string;
};
...
export default function VehicleDetailsClient({ vehicleId, embedded = false, startEditing = false, initialData, onChanged }: { vehicleId: string; embedded?: boolean; startEditing?: boolean; initialData?: VehicleProfilePayload | null; onChanged?: () => void }) {
  ...
  return <main className={embedded ? styles.embedded : styles.page}>
    ...
    <section className={styles.summaryGrid}>
      {!vehicle.isGuest && <SummaryCard icon={ShieldCheck} label="Documents" value={`${data.documents.length}/${DOCUMENT_TYPES.length} recorded`} />}
      {!vehicle.isGuest && <SummaryCard icon={Wrench} label="Open maintenance" value={String(openMaintenance)} />}
      <SummaryCard icon={CarFront} label={vehicle.isGuest ? "Guest Car history" : "Rental history"} value={`${data.rentals.length} rental${data.rentals.length === 1 ? "" : "s"}`} />
      {!vehicle.isGuest && <SummaryCard icon={IndianRupee} label="Vehicle expenses" value={money(totalExpenses)} />}
    </section>

    <nav className={styles.tabs} aria-label="Vehicle detail sections">
      {!vehicle.isGuest && <button className={tab === "documents" ? styles.activeTab : ""} onClick={() => setTab("documents")}>Documents</button>}
      {!vehicle.isGuest && <button className={tab === "maintenance" ? styles.activeTab : ""} onClick={() => setTab("maintenance")}>Maintenance</button>}
      {!vehicle.isGuest && <button className={tab === "tyres" ? styles.activeTab : ""} onClick={() => setTab("tyres")}>Tyres</button>}
      <button className={tab === "history" ? styles.activeTab : ""} onClick={() => setTab("history")}>History</button>
    </nav>

    {!vehicle.isGuest && tab === "documents" && <ReadDocuments rows={data.documents} />}
    {!vehicle.isGuest && tab === "maintenance" && <ReadMaintenance rows={data.maintenance} />}
    {!vehicle.isGuest && tab === "tyres" && <ReadTyres rows={data.tyres} warning={data.tyreWarning ?? null} />}
    {tab === "history" && <ReadHistory rentals={data.rentals} expenses={vehicle.isGuest ? [] : data.expenses} />}
  </main>;
}
...
function EditVehicleForm({ data, onCancel, onSaved }: { data: VehicleProfilePayload; onCancel: () => void; onSaved: () => Promise<void> }) {
  ...
  const automaticStatusLocked = ["rented", "booked"].includes(v.status.toLowerCase());
  const [operationalStatus, setOperationalStatus] = useState(["available", "inactive", "maintenance"].includes(v.status.toLowerCase()) ? v.status.toLowerCase() : "available");
  ...
  async function save() {
    if (v.isGuest && (!base.guestOwnerName.trim() || !base.guestOwnerPlace.trim())) { setError("Guest Car owner name and place are required."); return; }
    setSaving(true); setError(null);
    try {
      const response=await fetch(`/api/vehicles/${v.id}`,{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify(automaticStatusLocked ? base : { ...base, status: operationalStatus })}); const payload=await readJson<{ok:boolean;error?:string}>(response); if(!response.ok||!payload.ok) throw new Error(payload.error??"Could not update vehicle.");
