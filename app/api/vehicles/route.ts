...
const manualVehicleStatus = (value: unknown, isGuest: boolean) => {
  ...
  const allowed = isGuest ? ["available", "inactive"] : ["available", "inactive", "maintenance"];
  if (allowed.includes(normalized)) return normalized;
  throw new RequestError(isGuest ? "Guest Car status can only be Active or Inactive." : "Vehicle status can only be Active, Inactive or Maintenance.");
};
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const status = manualVehicleStatus(body.status, isGuest);
    ...
    return Response.json({ ok: true, vehicle: { id: saved.id, name: saved.name, plate: saved.registrationNumber, isGuest: saved.isGuest } }, { status: 201 });
  } catch (error) {
