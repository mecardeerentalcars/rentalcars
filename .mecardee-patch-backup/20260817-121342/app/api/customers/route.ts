...
export async function POST(request: Request) {
  try {
    ...
    const [saved] = await db.insert(customers).values({ name, phone, whatsappNumber, drivingLicence, city }).returning();
    return Response.json({ ok: true, customer: { id: saved.id, name: saved.name, phone: saved.phone } }, { status: 201 });
  } catch (error) {
